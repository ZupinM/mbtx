...
PACK(typedef struct t_FrSkyChannelData
{
	union opt
	{
    	...
    	uint8_t   unused_alarms_level:4;
    	uint8_t   unused_alarms_greater:2;     // 0=LT(<), 1=GT(>)
    	uint8_t   type:2;               // 0=volts, 1=raw, 2=volts*2, 3=Amps
		} alarm ;
		struct scale
		{
    	uint8_t   ratio ;       	// 0.0 means not used, 0.1V steps EG. 6.6 Volts = 66. 25.1V = 251, etc.
    	uint8_t   unused_offset ;    	  // 0.0 means not used, 0.1V steps EG. 6.6 Volts = 66. 25.1V = 251, etc.
			uint8_t   unused_units ;					// 0.1V steps EG. 6.6 Volts = 66. 25.1V = 251, etc.
    	uint8_t   unused_multiplier:3 ;  // 1, 2, 3 or 4
    	uint8_t   unused_unused1:1 ;
    	uint8_t   unused_decimals:2 ;		// 0, 1 or 2
    	uint8_t   unused_type:2;       	// 0=volts, 1=raw, 2=volts*2, 3=Amps
		} scale ;
	} opt ;
}) FrSkyChannelData;
...
PACK(typedef struct t_FrSkyData {
    FrSkyChannelData channels[2];
		uint8_t unused_voltSource:2 ;
		uint8_t unused_ampSource:2 ;
...
#ifdef V2
extern V2ModelData g_model;
extern V2EEGeneral g_eeGeneral;
#else
